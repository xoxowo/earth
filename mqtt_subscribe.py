...
def on_message(client, userdata, msg):
    ...
    for i in range(detection_count):
        ...
        mycursor.execute('SELECT id FROM states WHERE state=%s', (state,))
        state1, = mycursor.fetchone()
        # print(state1)
      
        sql = '''INSERT 
            INTO detections (x,y,width,height,datetime,cam_id,detection_type_id,state_id) 
            VALUES (%s, %s, %s, %s,%s, %s, %s, %s)
            '''
        val = (x,y,width,height,datetime1,cam_id, detection_type1, state1)
...
client = mqtt.Client(client_id="wecode-melony2") # client_id는 생략 가능
